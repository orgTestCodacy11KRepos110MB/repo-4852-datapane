...
@multimethod
def convert_to_block(x: t.Any) -> b.DataBlock:
    return b.Attachment(x)


@multimethod
def convert_to_block(x: str) -> b.DataBlock:
    return b.Text(x)


@multimethod
def convert_to_block(x: Path) -> b.DataBlock:
    return b.Attachment(file=x)


@multimethod
def convert_to_block(x: pd.DataFrame) -> b.DataBlock:
    n_cells = x.shape[0] * x.shape[1]
    return b.Table(x) if n_cells <= TABLE_CELLS_LIMIT else b.DataTable(x)
...
# Plots
@multimethod
def convert_to_block(x: SchemaBase) -> b.DataBlock:
    return b.Plot(x)
...
if HAVE_BOKEH:
    ...
    @multimethod
    def convert_to_block(x: t.Union[BFigure, BLayout]) -> b.DataBlock:
        return b.Plot(x)
...
if HAVE_PLOTLY:
    ...
    @multimethod
    def convert_to_block(x: PFigure) -> b.DataBlock:
        return b.Plot(x)
...
if HAVE_FOLIUM:
    ...
    @multimethod
    def convert_to_block(x: Map) -> b.DataBlock:
        return b.Plot(x)
...
if HAVE_PLOTAPI:
    ...
    @multimethod
    def convert_to_block(x: Visualisation) -> b.DataBlock:
        return b.Plot(x)
...
if HAVE_MATPLOTLIB:
    ...
    @multimethod
    def convert_to_block(x: t.Union[Figure, Axes, ndarray]) -> b.DataBlock:
        return b.Plot(x)
